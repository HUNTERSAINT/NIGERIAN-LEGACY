...
from bot.cogs.setup_system import ensure_roulette_channel
...
class NigeriaBot(commands.Bot):
    def __init__(self):
        ...
        super().__init__(
            command_prefix="!",
            intents=intents,
            description="🇳🇬 Nigerian Legacy RP economy and games bot",
        )
    ...
    async def on_ready(self):
        ...
        for guild in self.guilds:
            try:
                channel, created = await ensure_roulette_channel(guild)
                if created:
                    logger.info("Roulette channel created in %s (#%s)", guild.name, channel.name)
                    await channel.send(
                        embed=discord.Embed(
                            title="🎰 Nigerian Legacy Roulette",
                            description=(
                                "The wheel is ready. Use `/roulette` to place a spin "
                                "or `!roulette <bet> <amount>` to play."
                            ),
                            color=0xC0392B,
                        )
                    )
                else:
                    logger.info("Roulette channel ready in %s (#%s)", guild.name, channel.name)
            except discord.Forbidden:
                logger.warning(
                    "Could not create the roulette channel in %s; "
                    "the bot needs Manage Channels permission.",
                    guild.name,
                )
            except Exception:
                logger.exception("Failed to prepare the roulette channel in %s", guild.name)
        await self.change_presence(
            activity=discord.Activity(
                type=discord.ActivityType.watching,
                name="🇳🇬 Nigerian Legacy RP | /help",
            )
        )
