    ...
    for name in REQUIRED_DISCORD_ROLES:
        ...
        try:
            await guild.create_role(
                name=name,
                mentionable=True,
                reason="Nigerian Legacy RP standard role setup",
            )
            created.append(name)
...
class RoleSetup(commands.Cog):
    ...
    async def on_guild_join(self, guild: discord.Guild):
        ...
        try:
            ...
            if channel and channel.permissions_for(guild.me).send_messages:
                await channel.send(
                    f"🇳🇬 Nigerian Legacy roles are ready. Created **{len(created)}** missing roles. "
    ...
    @app_commands.command(
        name="setup-roles",
        description="[Admin] Create all standard Nigerian Legacy RP roles that are missing.",
    )
