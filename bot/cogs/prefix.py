...
class Prefix(commands.Cog):
    ...
    async def cmds(self, ctx):
        ...
        embed = discord.Embed(
            title="🇳🇬  Nigerian Legacy RP — All Commands",
        ...
        embed.add_field(name="⚽ Betting", value=(
            ...
            "`!bet` — browse fixtures and build a visible multi-match slip\n"
            "`!bethistory` · `!betstats`\n"
        ...
        embed.add_field(name="🎰 Games", value=(
            "`!roulette <red/black/odd/even> <amount>` *(#roulette only)*\n"
            "`!roulette number <amount> <0-36>` *(#roulette only)*\n"
            "`!roulettetoggle <on/off>` *(Admin)*\n"
            "`!rob @player` — attempt a risky wallet robbery"
        ), inline=False)

        embed.add_field(name="🆘 Support", value=(
            "`!ticket [subject]` — open a private support channel\n"
        ...
        embed.add_field(name="🇳🇬 Setup & Immigration", value=(
            "`!setup` — create Nigerian Legacy categories, channels, roles, and logs *(Admin)*\n"
