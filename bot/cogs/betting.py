...
class Betting(commands.Cog):
    ...
    async def _run_one_match(self):
        ...
        if winners:
            winner_lines = "\n".join(
                f"<@{uid}> bet {fmt(amt)} → won **{fmt(payout)}** (+{fmt(profit)})"
    ...
    @app_commands.command(name="bet", description="Place a bet on the current virtual match.")
    @app_commands.describe(
        choice="home, draw, or away",
        amount="Amount in Naira to bet.",
    )
    async def bet(self, interaction: discord.Interaction, choice: str, amount: int):
